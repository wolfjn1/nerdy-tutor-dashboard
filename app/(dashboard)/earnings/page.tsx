  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Earnings & Billing</h1>
          <p className="text-gray-600 mt-1">Track your income and manage payments</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-4 rounded-lg bg-green-50 border border-green-200">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-green-700">Current Period</span>
          ...
          <div className="p-4 rounded-lg bg-blue-50 border border-blue-200">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-blue-700">Unpaid Balance</span>
              <CreditCard className="w-4 h-4 text-blue-600" />
            </div>
            <div className="text-2xl font-bold text-gray-900">${earnings.unpaid.toFixed(2)}</div>
          ...
          <div className="p-4 rounded-lg bg-yellow-50 border border-yellow-200">
            ...
            <div className="text-2xl font-bold text-gray-900">${earnings.pending.toFixed(2)}</div>
          ...
          <div className="p-4 rounded-lg bg-purple-50 border border-purple-200">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-purple-700">Avg Per Session</span>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Uninvoiced Sessions</h2>
            <p className="text-sm text-gray-600 mt-1">Select sessions to create an invoice</p>
          </div>
        ...
        {uninvoicedSessions.length > 0 ? (
            ...
            {uninvoicedSessions.map((session) => (
              ...
              >
                ...
                <div className="text-right">
                  <p className="font-medium text-gray-900">${session.amount.toFixed(2)}</p>
                  <p className="text-xs text-gray-600">${session.rate}/hr</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="text-center py-8">
            ...
            <p className="text-gray-600">All sessions have been invoiced!</p>
          </div>
        )}
      </div>
            ...
            <div key={invoice.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">{invoice.studentName} • {invoice.sessions} sessions</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="text-right">
      ...
      {/* Upcoming Payouts */}
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Upcoming Payouts</h2>
        <div className="space-y-3">
            ...
            <div key={payout.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              ...
              <div className="text-right">
                ...
                <Badge variant="secondary" size="sm" className={
                  payout.status === 'scheduled' ? 'bg-blue-50 text-blue-700 border-blue-200' :
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="text-center py-3 px-2 text-sm font-medium text-gray-700">Invoice</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredTransactions.map((transaction) => (
                    <tr key={transaction.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-2 text-right">
                        <p className={cn(
                          "text-sm font-medium",
                          transaction.type === 'payout' ? 'text-blue-600' : 'text-gray-900'
                        )}>
                      ...
                      <td className="py-3 px-2 text-center">
                        {transaction.type === 'session' && (
                          transaction.invoiced ? (
                            <Badge variant="secondary" size="sm" className="bg-green-50 text-green-700 border-green-200">
                              <Check className="w-3 h-3 mr-1" />
                              Invoiced
                            </Badge>
                          ) : (
                            <Badge variant="secondary" size="sm" className="bg-gray-50 text-gray-600 border-gray-200">
                              Pending
                            </Badge>
                          )
                        )}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium text-gray-900">Direct Deposit - Bank of America</p>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            <h3 className="font-medium text-gray-900 mb-3">Selected Sessions</h3>
            <div className="space-y-2">
              {uninvoicedSessions
                .filter(s => selectedSessions.includes(s.id))
                .map(session => (
                  <div key={session.id} className="flex justify-between text-sm">
                    ...
                    <span className="font-medium text-gray-900">${session.amount.toFixed(2)}</span>
                  </div>
                ))}
            </div>
            <div className="mt-3 pt-3 border-t border-gray-200 flex justify-between">
              <span className="font-medium text-gray-900">Total</span>
              <span className="font-bold text-lg text-gray-900">${calculateSelectedTotal().toFixed(2)}</span>
            </div>
          </div>
          ...
          <div>
            ...
            <select className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-purple-500">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-gray-600">Issued: {new Date(selectedInvoice.date).toLocaleDateString()}</p>
              </div>
              <Badge 
                variant="secondary" 
                size="lg" 
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-3">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-900">{selectedInvoice.studentName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Sessions:</span>
                <span className="font-medium text-gray-900">{selectedInvoice.sessions}</span>
              </div>
              <div className="flex justify-between">
