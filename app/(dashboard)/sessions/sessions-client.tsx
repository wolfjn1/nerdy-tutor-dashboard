...
export default function SessionsClient({ sessions }: SessionsClientProps) {
          ...
          <div className="space-y-2 mb-4 text-sm">
            <div>
              ...
              <span className="ml-2 text-gray-900 dark:text-gray-100">
                {formatDate(new Date(session.date))} at {formatTime(new Date(session.date))}
              </span>
            </div>
  ...
  const CalendarDay = ({ date, sessions }: { date: Date, sessions: Session[] }) => {
      ...
      <div className={cn(
        ...
        <div className="space-y-1">
          {sessions.slice(0, 3).map((session) => (
            ...
            >
              <div className="flex items-center gap-1">
                <span className="font-medium">{formatTime(new Date(session.date))}</span>
                <span className="truncate">{session.student?.firstName || 'Unknown'}</span>
              </div>
            </div>
          ))}
          {sessions.length > 3 && (
                ...
                return (
                  ...
                  >
                    {sessions.map((session) => (
                      ...
                      >
                        <div className="truncate">{session.student?.firstName || 'Unknown'}</div>
